...
(function() {
    ...
    let initCountdown = 10; // 10 secondi di setup
    
    let dbCooldownSeconds = 120; // 2 Minuti esatti (120 secondi)
    let completedSweeps = 0;     // Contatore dei giri 
    ...
    function deepClearFilters() {
        console.log("[YOS-SYNC] 🧹 Deep Clear: elimino tutti i filtri testo e clicco le 'X'...");
    ...
    function injectConsDashboard() {
        ...
        let dashboard = document.getElementById('cons-dashboard');
        if (!dashboard) {
            dashboard = document.createElement('div');
            dashboard.id = 'cons-dashboard';
            
            const btn = document.createElement('button');
            btn.id = 'cons-autoscan-btn';
            btn.innerHTML = '🔄 Auto-Scan: ON';
            btn.onclick = () => {
                
                // SEGNALE DI HARD RESET (Scatta quando è fermo e le condizioni sono rispettate)
                if (!isAutoScanActive && dbCooldownSeconds <= 0 && completedSweeps >= 2) {
                    console.log("[YOS-SYNC] 🔄 Riavvio Completo: pulizia profonda e ricaricamento...");
                ...
                // NORMALE PAUSA / PLAY DURANTE IL CICLO
        ...
        // Aggiorna la dicitura della UI in base alla doppia logica
        const infoBox = document.getElementById('cons-timer-box');
        if (infoBox) {
            if (isInitializing) {
                ...
                infoBox.innerHTML = "<span class='text-ready'>IN PAUSA: Dati Pronti (2+ Giri completati)</span>";
                ...
                if (dbCooldownSeconds <= 0 && completedSweeps < 2) {
                    infoBox.innerHTML = "<span class='text-warning'>Attesa fine cicli... (" + completedSweeps + "/2)</span>";
                } else {
    ...
    function runSetupSequence() {
        ...
        setTimeout(() => {
            ...
            if (hoursSelect) {
                forceAggressiveClick(hoursSelect);
                setTimeout(() => clickOptionByText(targetHours + ' hours'), 500);
            }
        }, 4000);
        ...
        setTimeout(() => {
            ...
            if (unitSelect) {
                if (targetUnit === "NONE") {
                    ...
                    setTimeout(() => clickOptionByText(targetUnit), 500);
                }
            }
        }, 6000);
        ...
        setTimeout(() => {
            ...
            if (paginatorSelect) {
                forceAggressiveClick(paginatorSelect);
                setTimeout(() => clickOptionByText('100'), 500);
            }
        }, 8000);

        setTimeout(() => {
            console.log("[YOS-SYNC] ✅ Setup Completato! Partenza Immediata.");
    ...
    setInterval(() => {
        ...
        if (isInitializing && hasInitializedFilters) {
            ...
            // Controllo "Doppia Condizione": Tempo esaurito + Almeno 2 Giri conclusi
            else if (dbCooldownSeconds <= 0 && completedSweeps >= 2) {
                console.log("[YOS-SYNC] ⏱️ Obiettivi raggiunti (Tempo e Giri). Scansione in pausa.");
                isAutoScanActive = false; // Questo ferma i click di scan
                
                const btn = document.getElementById('cons-autoscan-btn');
                if (btn) {
                ...
                // -------------------------------------------------------------
                // NOVITÀ: Click sul bottone "Reset filter" per sbloccare la UI
                // -------------------------------------------------------------
                console.log("[YOS-SYNC] 🔄 Clicco il tastino 'Reset filter' per preparare la UI ai controlli manuali...");
                const finalRefreshBtn = document.querySelector('button[title="Reset filter"]');
                if (finalRefreshBtn) {
                    forceAggressiveClick(finalRefreshBtn);
                }
            }
        }
    }, 1000);
        ...
        if (!isRewinding) {
            ...
            rows.forEach(row => {
                ...
                if (pieceCountIndex > -1 && cells[pieceCountIndex]) {
                    ...
                    const pieceNode = row.querySelector('td.mat-column-pieceCount') || row.querySelector('td[class*="pieceCount" i]');
                    if (pieceNode) pieceCount = parseInt(pieceNode.innerText.replace(/\D/g, ''), 10) || 0;
                }
            ...
            if (!isPrevDisabled) {
                ...
                // FINE RITORNO = 2° GIRO COMPLETATO
                isRewinding = false;
                completedSweeps++;
                const refreshBtn = Array.from(document.querySelectorAll('button')).find(b => b.innerText && b.innerText.includes('Refresh CONS Data'));
                forceAggressiveClick(refreshBtn); lastClickTime = Date.now();
    ...
    function renderYosRemoteControl() {
        if (!document.title.includes('CONS Maintenance')) {
            let controlPanel = document.getElementById('yos-remote-control');
            if (!controlPanel) {
                controlPanel = document.createElement('div');
                controlPanel.id = 'yos-remote-control';
                ...
                document.getElementById('yos-hours-select').addEventListener('change', function(e) {
                    GM_setValue('cons_target_hours', e.target.value); GM_setValue('cons_pending_hours_command', e.target.value);
                });
                document.getElementById('yos-unittype-select').addEventListener('change', function(e) {
                    GM_setValue('cons_target_unittype', e.target.value); GM_setValue('cons_pending_unittype_command', e.target.value);
                });
            }

            let statusBadge = document.getElementById('yos-sync-status');
            if (!statusBadge) {
                statusBadge = document.createElement('div');
                statusBadge.id = 'yos-sync-status';
            ...
            if (lastHeartbeat === 0) {
                ...
                statusBadge.style.backgroundColor = '#dc3545'; statusBadge.style.color = 'white'; textEl.innerText = "🔴 CONNESSIONE PERSA (DB Congelato)";
            }
        }
    }
            ...
            if (trailerId !== "" && currentConsTrailers[trailerId]) {
                ...
                if (records.length > 0) {
                    ...
                    if (trailers.length > 1) {
                        targetText = "!";
                        targetBg = "rgba(220, 53, 69, 0.9)"; 
                        targetColor = "white";
                    } else if (bulks.length > 0) {
                        targetText = "?";
                        targetBg = "rgba(255, 193, 7, 0.9)"; 
                        targetColor = "black";
                    } else if (trailers.length === 1) {
                        targetText = "✅";
                        targetBg = "rgba(40, 167, 69, 0.9)"; 
                        targetColor = "white";
                    }
                }
            } 
