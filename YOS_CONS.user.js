// ==UserScript==
// @name         YOS & CONS Sync Overlay (Zone 300/400) - PRO
// @namespace    http://tampermonkey.net/
// @version      1.2
// @description  Single-Check su trazioni chiuse. Cache Tracker (30 min border). Restart remoto YOS.
...
(function() {
    ...
    let dbCooldownSeconds = 120; // 2 Minuti esatti
    ...
    function runSetupSequence() {
        ...
        setTimeout(() => {
            ...
            if (originSelect) { forceAggressiveClick(originSelect); setTimeout(() => clickOptionByText('IMRH'), 500); }
        }, 2000);

        setTimeout(() => {
            const hoursSelect = document.querySelector('mat-select[formcontrolname="hours"]');
            if (hoursSelect) { forceAggressiveClick(hoursSelect); setTimeout(() => clickOptionByText(targetHours + ' hours'), 500); }
        }, 4000);
    ...
    setInterval(() => {
        if (document.title.includes('CONS Maintenance')) {
            // Controlla Comando Remoto Hard Reset
            ...
            if (isInitializing && hasInitializedFilters) {
                ...
                if (dbCooldownSeconds > 0) {
                    ...
                    GM_setValue('cons_scan_state', 'COMPLETED'); // Segnala a YOS che è finito bene
    ...
    function processSingleCheckQueue() {
        if (isProcessingCommand) return true; // Blocca se sta già lavorando
        ...
        setTimeout(() => {
            // 2. Cerca l'input per il Trailer ID
            ...
            setTimeout(() => {
                ...
                // 4. Attende e poi legge i dati solo per lui
                setTimeout(() => {
                    let newRecords = [];
                    ...
                    rows.forEach(row => {
                        ...
                        if (pieceCountIndex > -1 && cells[pieceCountIndex]) pieceCount = parseInt(cells[pieceCountIndex].innerText.replace(/\D/g, ''), 10) || 0;
                        
                        if (consId !== '') newRecords.push({ state, assetId, unitType, pieceCount, consId });
                    });

                    // Sovrascrive solo questo trailer
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            if (!isAutoScanActive && dbCooldownSeconds <= 0 && completedSweeps >= 2) GM_setValue('cons_scan_state', 'COMPLETED');
                        }, 1000);
                    }, 500);
    ...
    function scanConsMaintenance() {
        ...
        // SE C'È UN SINGLE CHECK IN CODA, FERMA LO SWEEP E FALLO
    ...
    function renderYosRemoteControl() {
        if (!document.title.includes('CONS Maintenance')) {
            ...
            if (lastHeartbeat === 0) {
                ...
                // Modifica per mostrare l'ora di completamento
    ...
    function injectIntoYosContainers() {
        ...
        const lastHeartbeat = GM_getValue('cons_last_heartbeat', 0);
        const isCacheOld = (Date.now() - lastHeartbeat) > 1800000; // 30 Minuti in millisecondi
        ...
        containers.forEach(container => {
            ...
            // ==========================================
            // LOGICA SINGLE CHECK SU TRAZIONI PRONTE
            // ==========================================
            ...
            if (isReady && trailerId !== "") {
                ...
                if (!queuedList.includes(trailerId)) {
                    ...
                    if (queuedList.length > 200) queuedList.shift(); // Evita array enormi
            ...
            let targetBorder = isCacheOld ? "2px solid red" : "none"; // Bordo rosso se cache > 30 min
